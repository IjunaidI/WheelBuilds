...
- `src/modules/vendor-sync/` — Inventory sync pipeline that pulls CSV feeds from the WheelPros distributor, diffs against the last applied state, and writes products + per-warehouse inventory levels into Medusa. Two adapter instances ship today: `wheelpros-wheels` and `wheelpros-tires`. Architecture: four MikroORM tables (`vendor_feed_run`, `vendor_feed_staging`, `vendor_stock_staging`, `vendor_product_current`), a `VendorAdapter` interface with discriminated-union `NormalizedRecord` (wheel | tire), a sequential per-process apply loop wrapped in try/catch **per product group** (not per part_number — see grouping note below), and Medusa 2.0 core flows for every catalog mutation. Runs every 12 hours via the cron at `src/jobs/vendor-sync-tick.ts`. Image-less rows are filtered at staging (vendor CDN images pass through to `thumbnail`) — and since WB-115 that gate also checks **reachability**, not just non-emptiness (see below). For the full picture, read [`vendor-sync-implementation`](docs/reference/vendor-sync-implementation.md).
  - **Grouping (wheels only).** Multiple CSV rows that share `Brand + DisplayStyleNo + Finish` collapse into ONE Medusa product with N variants, where the variant axes are Bolt Pattern, Diameter, Width, and Offset. The `group_key` column on `vendor_feed_staging` and `vendor_product_current` is what carries this relationship. Rows with empty `DisplayStyleNo` fall back to a per-SKU key (`sku:<partNumber>`) so unrelated rows from the same brand never merge. The `productType` discriminator routes records to the wheel vs tire grouping path.
  - **Grouping (tires — WB-005 SP1).** Tires group by `Brand + <extracted model>` (the model is scraped from the free-text `PartDescription` by `wheelpros-tires/model-key.ts`, per-SKU fallback when no confident model survives), with the canonical **size** (e.g. `305/45R22 118S`) as the single variant axis. Pure helpers live in `pipeline/tire-facets.ts` (canonical size / label / tire-type) + `pipeline/tire-grouping.ts` (options / axis / dedupe, mirroring `wheel-grouping.ts`); `applyNewTireGroup` builds the multi-variant product; `buildSearchDocument`'s tire branch (`buildTireDocument`) indexes them as `product_type = "tire"` with size/rim/width/aspect/load/speed/type facets (registered in `medusa-config.js`). Storefront tire discovery + PDP (SP2/SP3) are not built yet.
  ...
  - **Image reachability gate (WB-115).** `stageSkipReason` drops a row when its image URL is empty (`"no-image"`) **or when the URL doesn't resolve** (`"image-unreachable"`). The distinction matters: a live probe found 664 of 2,852 indexed products (23%) pointed at a WheelPros URL that 404s while **zero** had an empty thumbnail — an emptiness-only check catches none of them. `pipeline/image-reachability.ts` HEAD-probes deduplicated URLs (concurrency 24, 10s timeout) behind the `vendor_image_check` table: successes are cached 7 days, **known-dead URLs are re-probed every run** so a product reappears automatically once the vendor publishes its image. Nothing new delists anything — a dropped row simply looks absent to `diff.ts`, so the existing paths take over (all rows gone → `discontinuedGroups` → product drafted → the Meili plugin `deleteDocument`s it; some rows gone → `removed_part_numbers` → just those variants discontinued, which is how a wheel keeps the finishes whose images are fine).
    - **⚠️ This gate can empty the catalog if you loosen it. It is deliberately FAIL OPEN: only HTTP 404/410 marks an image dead.** Timeouts, DNS failures, 5xx, 429, a synchronous throw, and any URL missing from the checker's result map all count as *reachable*. `undefined` means reachable — that is why `stageSkipReason` tests `imageReachable === false` and not `!== true`. A circuit breaker fails the whole run when more than `VENDOR_SYNC_IMAGE_DEAD_MAX_RATIO` (default `0.40`) of **deduplicated URLs** come back dead; count by unique URL, never per row (one shared placeholder backs dozens of products, and at ~17 rows/product a handful of placeholders would trip a per-row breaker and freeze every sync). A malformed ratio env falls back to the default with a warning rather than silently disabling the breaker. Kill switch: `VENDOR_SYNC_IMAGE_CHECK_ENABLED=false`.
    - Two states silently disable checking (fail-safe, but quiet): module options with no `imageCheck` block, and a stale `.medusa/server` serving a pre-WB-115 `medusa-config.js`. Nothing logs "image check disabled" — if a run reports 0 checked, suspect the stale-config trap first.
  - **Search subdir (`src/modules/vendor-sync/search/`).** Home of the Meilisearch transformer for wheels: `bolt-pattern-canonical.ts` (pure `canonicalBoltPatterns(input)` → `["{count}x{pcd_mm}"]`, snaps to nearest standard PCD — this is the fitment join key with wheel-size.com), `normalize-finish.ts` (pure `normalizeFinish(raw)` → `"black"|"bronze"|"silver"`, precedence: bronze > black-dominance > silver > black default), and `build-search-document.ts` (`buildSearchDocument(product)` → flat indexed `WheelSearchDocument` or `null` for non-wheels; `price_min`/`price_max` are INTEGER CENTS via `Math.round(major*100)`). `medusa-config.js` calls `buildSearchDocument` from the plugin `transformer`. NOTE (WB-030): `normalize-finish.ts` has a twin on the storefront PDP at `storefront/src/lib/fitment/normalize-finish.ts` (logically equivalent — backend uses keyword arrays, storefront uses regex). They are guarded against drift by the shared `fixtures/finish-normalize-golden.json`, asserted by a test in EACH app; change the golden + both copies together rather than hand-syncing silently.
...
- Discovery is server-rendered; the legacy Algolia `react-instantsearch-hooks-web` client (`src/lib/search-client.ts`) was deleted in WB-096.
