...
- follow-ups: `skipped_image_unreachable_count` isn't persisted to `vendor_feed_run` (admin console under-reports); no global time budget, so a *blackholing* CDN costs ~3.5 min per 500-row batch; `imageCheck.ttlDays/concurrency/timeoutMs` are plumbed but not populated from `medusa-config.js` (defaults only, no env knob); dead-image SKUs' stock freezes during the park window while still buyable; `truncateVendorState`/dev-wipe don't clear `vendor_image_check`; nothing logs when the gate is disabled.
