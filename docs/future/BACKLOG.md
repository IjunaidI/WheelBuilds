...
- **G6 · Catalog breadth & pricing** `[L–XL · WB-005 is a big spec alone]` — **WB-005 CODE-COMPLETE (2026-07-02): all 3 tire sub-projects done — SP1 (backend grouping+indexing, merged) + SP2 (tire discovery `/tires`, merged) + SP3 (tire PDP, on `feat/tire-pdp`).** Tires group by Brand+model (size = variant axis), index as `product_type = "tire"`, browse at a faceted `/tires`, and open a tire PDP (rim-chips size selector → add-to-cart). Remaining for WB-005: the **prod cutover** (tire feed apply). Then markup/MAP/margin pricing (WB-024), de-hardcode bootstrap identity + vendor roster (WB-025/026). → WB-005, WB-024, WB-025, WB-026
...
- verify: after the tire feed apply, tires are grouped Medusa products with size variants; Meili has `product_type = "tire"` docs + facets; `/tires` renders a faceted catalog and a tire card → a tire PDP with a size selector + working add-to-cart. **Gates met: SP1 backend 288 ; SP2+SP3 storefront vitest 153 (28 files), tsc 0-new, routes compile. Live tire browse/PDP + the dry-run/apply DEFERRED → the cutover (needs tire products in the DB/index).**
- refs: SP1 [spec](../in-progress/specs/2026-07-02-tire-store-design.md)+[plan](../in-progress/plans/2026-07-02-tire-store-backend.md) ; SP2 [spec](../in-progress/specs/2026-07-02-tire-discovery-design.md)+[plan](../in-progress/plans/2026-07-02-tire-discovery.md) ; SP3 [spec](../in-progress/specs/2026-07-02-tire-pdp-design.md)+[plan](../in-progress/plans/2026-07-02-tire-pdp.md)
