...
### WB-115 · Products with a dead vendor image still show and index   [CODE COMPLETE — awaiting live apply]
- status: in-progress (code merged + fully reviewed; the live dry-run + apply gate has NOT run)
- area: backend/vendor-sync (staging gate) + storefront/product-detail (PDP finish filter)
- evidence: live probe 2026-07-20 over the whole Meilisearch index — 2,852 wheel/tire docs, 1,896 unique thumbnail URLs, **343 of them 404 → 664 docs (23%) affected** (267 wheels, 397 tires). **0 docs have an EMPTY thumbnail.** Worst shared placeholders: `m500/mTIRE.png` (64 docs), `mTYOPCQ.png` (48), `mFTWPK4.png` (36), `mMIPMT.png` (21).
- problem: both guards asked the wrong question. Backend `stageSkipReason` was `if (!normalized.imageUrl)` and storefront `hasImage()` is a string-emptiness test — neither checks whether the image RESOLVES. Since no product has an empty thumbnail, the gates caught nothing and image-less products rendered as blank cards across discovery, the home NEW THIS WEEK row, and PDP. A user request 2026-07-20.
- decisions (user-approved): (1) hide ALL 664, wheels and tires (catalog 2,852 → ~2,188; reversible — a product returns automatically once the vendor publishes its image); (2) per-finish granularity — drop only the dead finish's variants, delist the product only when EVERY row is dead.
- fix: reachability gate at staging. `stageSkipReason` gains `imageReachable?` + reason `"image-unreachable"`; `createImageReachabilityChecker` HEAD-probes deduped URLs (concurrency 24, 10s timeout) behind a persistent `vendor_image_check` cache (7-day TTL on successes; **known-dead re-checks every run** so recovery is automatic). Everything after a dropped row is pre-existing: the diff sees it absent → all-rows-gone becomes `discontinuedGroups` → product drafted → the Meili plugin `deleteDocument`s it (satisfying "shouldn't even be indexed"); some-rows-gone becomes `removed_part_numbers` → only those variants discontinued. Plus: the changed-group path now RECOMPUTES product `thumbnail`/`images` from surviving rows, and the PDP stops offering discontinued finishes.
- **safety (this feature can empty the store if wrong):** FAIL OPEN — only HTTP 404/410 marks an image dead; timeout, DNS failure, 5xx, 429, sync throw, and any URL missing from the result map all count as reachable. Circuit breaker fails the run when the dead/checked ratio over **deduplicated URLs** exceeds a **per-vendor** threshold — wheels 0.40, tires 0.70 (measured 2026-07-20 on the full untruncated feeds: wheels 8.0%/313-of-3,914 dead, tires 47.7%/103-of-216 dead — tires genuinely ship dead placeholder URLs for about half their lines, a true baseline not a checker malfunction, so tires needs a higher ceiling than wheels to avoid permanently tripping while still catching a catastrophic ~100% CDN failure). Resolution order is per-vendor env (`VENDOR_SYNC_IMAGE_DEAD_MAX_RATIO_WHEELS`/`_TIRES`) → global env (`VENDOR_SYNC_IMAGE_DEAD_MAX_RATIO`) → the hardcoded per-vendor default (per-row counting was rejected: ~17 rows/product means one placeholder backing 64 products = ~4% of the feed from a single probe, so ~10 would trip it and freeze all inventory sync). Malformed ratio env → `Number.isFinite` fallback + warn, so a typo can't silently disable the breaker. Kill switch `VENDOR_SYNC_IMAGE_CHECK_ENABLED=false`.
- done (code): 2026-07-20 — SDD on `feat/wb-115-dead-image-exclusion` (3 impl tasks, per-task reviews [2× opus], opus whole-branch review, 2 fix waves). Tips: 1=`965978e`, 2=`12a4f0a`+`aaad70b`, 3=`8ae62fd`+`6923368`, final fixes `cab14a9`+`fb825b0`. Gate *at that point*: backend jest **450** + medusa build 0; storefront vitest **868** + tsc 2-baseline + next build 0. (The branch's FINAL gate after the two premerge rounds below is backend jest **488**.)
- **premerge fix pass (2026-07-20, same branch, still unmerged):** two live dry-runs measured the real feeds — wheels 8.0% dead (313/3,914 unique URLs), tires 47.7% dead (103/216, TRIPPED the 0.40 breaker; a TRUE baseline, not a malfunction — WheelPros ships dead placeholders for ~half their tire lines). Client decision: gate tires too despite hiding ~35%/397 of 1,131 tire products. Fixed: (1) `maxDeadRatio` overridable per vendor (wheels 0.40, tires 0.70 — not 1.0/disabled); (2) `shouldTrustImageChecks` minimum-sample floor (default 50) so a truncated feed can't misread as a catalog-wide crisis, with an explicit warn when the floor suppresses a would-be trip; (3) `isSampleFeedPath` now resolves-and-compares the real bundled-CSV absolute path instead of matching by basename, fixing a false "USING BUNDLED SAMPLE FEED" warning on a genuine live SFTP pull (WheelPros' real tire feed shares the bundled sample's filename); (4) `imageCheck.ttlDays/concurrency/timeoutMs` now wired to env (see follow-ups below — this closes that gap), guarded the same way as `maxDeadRatio`. Gate: backend jest **480** + medusa build 0 (storefront untouched).
- **premerge fix pass round 2 (2026-07-20, same branch, still unmerged) — an opus review of the round-1 premerge commit passed with 0 Critical but 2 Important + 3 Minor:** (1) *Important* — `medusa-config.js` hardcoded `maxDeadRatio` per vendor (0.40/0.70), which made the documented emergency knob `VENDOR_SYNC_IMAGE_DEAD_MAX_RATIO` inert for both live vendors; fixed via a new pure `resolveVendorMaxDeadRatio` (`lib/resolve-vendor-max-dead-ratio.ts`) resolving per-vendor env (`VENDOR_SYNC_IMAGE_DEAD_MAX_RATIO_WHEELS`/`_TIRES`, new) → global env → the hardcoded default; the existing `Number.isFinite` guard in `pipeline/stage.ts` still catches a malformed resolved value regardless of source. (2) *Important* — `isSampleFeedPath`'s `BUNDLED_SAMPLE_PATHS` used a hardcoded `__dirname`+5-ups walk that only reached the repo root from the `src/` layout; under `pnpm start` (the BUILT `.medusa/server` tree, one directory level deeper) the same walk landed inside `backend/.medusa/` — confirmed on disk (`backend/.medusa/wheelInvPriceData.csv` does not exist) — which meant `isSampleFeedPath` always returned false there and the `!allowSample` production safety guard could never fire. Fixed with `findBundledSampleDir`: walks up from the file's own `__dirname` checking (via `fs.existsSync`) for a directory containing BOTH bundled sample filenames together, which finds the real repo root in either layout regardless of cwd, without reintroducing basename-only matching (classification still compares a feedPath's own resolved path against the discovered directory's two known files, so a same-named tmpdir/SFTP download still doesn't match). Verified against an actual `medusa build` output by requiring the compiled `.medusa/server/.../resolve-feed.js` directly. (3) *Minor* — `resolveImageCheckNumericOption` accepted `0`/negatives for ttlDays/concurrency/timeoutMs even though its own docstring named `timeoutMs: 0` as the exact hazard it exists to prevent (instant-expiring probes fail-open every image to "alive", silently no-opping the gate); now requires `value > 0`. (4) *Minor* — the 7 `VENDOR_SYNC_IMAGE_*` env vars (5 original + 2 new per-vendor) were undocumented in `.env.template`/this doc; both now document all 7 with defaults. (5) *Minor* — `resolve-feed.test.ts` recomputed the bundled-sample path with the SAME hardcoded expression as the module under test (self-consistent, not proven real); added `fs.existsSync` assertions. Gate: backend jest **488** (`src/modules/vendor-sync` scope; was 480) + `medusa build` 0 (storefront untouched).
- **⚠️ REMAINING GATE — do NOT let the 12h cron apply this first.** Sequence: deploy backend (`init-backend` runs `Migration20260720120000`; clear `.medusa/server` or the stale-config trap silently disables the gate) → `pnpm vendor-sync:dry-run wheelpros-wheels` / `-tires` and read the `[image checks: N checked, M dead]` line → then the real full run. **The first enabled run drops ~23% of groups, which exceeds the pre-existing `discontinueThreshold` 0.05 and PARKS the run at `awaiting_approval`, blocking later FULL runs until an admin approves** (stock-only 3h runs are exempt and keep going). After apply, trigger a `meilisearch.sync` full reconcile so the drafted products are definitely evicted. No lockstep needed (the PDP filter is backward-compatible), but deploy the storefront to get the finish fix.
- follow-ups: `skipped_image_unreachable_count` isn't persisted to `vendor_feed_run` (admin console under-reports); no global time budget, so a *blackholing* CDN costs ~3.5 min per 500-row batch; dead-image SKUs' stock freezes during the park window while still buyable; `truncateVendorState`/dev-wipe don't clear `vendor_image_check`; nothing logs when the gate is disabled. ~~`imageCheck.ttlDays/concurrency/timeoutMs` plumbed but not populated from `medusa-config.js`~~ — fixed in the premerge fix pass above (`VENDOR_SYNC_IMAGE_TTL_DAYS`/`_CONCURRENCY`/`_TIMEOUT_MS`). ~~`VENDOR_SYNC_IMAGE_DEAD_MAX_RATIO` had no effect on either live vendor~~ — fixed in premerge fix pass round 2 above (per-vendor env override).
- **re-verified 2026-07-27 — STILL NOT APPLIED; the gate above is the whole reason dead-image products are still live.** User report: "there's still tires with no thumbnail being indexed." Live probe of the production Meilisearch index that day: 2,584 docs (1,449 wheel / 1,131 tire / 4 non-wheel stub); **0 tire docs have an empty thumbnail** — so `hasImage()` is doing its job and the complaint is *present-but-404* URLs, exactly the distinction this item exists for. HEAD-probing all 203 unique tire thumbnail URLs found **99 dead (48.8%)**, matching the 2026-07-20 baseline (103/216, 47.7%) — so the tires breaker (0.70) is NOT tripping and is not the blocker. The blocker is the REMAINING GATE line above: the drop exceeds `discontinueThreshold` 0.05, the run parks at `awaiting_approval`, and a parked run then blocks every later FULL run while the stock-only tick keeps running (the user's log shows exactly that — `[vendor-sync-stock-tick] Stock refresh for vendor: wheelpros-tires` with no full run). Operator action, no code change: approve via `POST /admin/vendor-sync/runs/:id/approve` (or raise `VENDOR_SYNC_DISCONTINUE_THRESHOLD` for the cutover run), then force a `meilisearch.sync` reconcile. Blocked 2026-07-27 by an unrelated Postgres storage-limit outage (see WB-116).
- refs: [in-progress/specs/2026-07-20-dead-image-exclusion.md](../in-progress/specs/2026-07-20-dead-image-exclusion.md) · [in-progress/plans/2026-07-20-dead-image-exclusion-plan.md](../in-progress/plans/2026-07-20-dead-image-exclusion-plan.md)

### WB-116 · Vendor-sync feed-parsing gaps + unreadable staging logs   [DONE — code]
- status: done (code; merged to local main). Discovered from a production sync log the user pasted 2026-07-27.
- area: backend/vendor-sync (`utils/tire-parse-helpers.ts`, `pipeline/stage.ts`, new `pipeline/normalization-error.ts`)
- problem 1 — **flotation tires silently lost their size.** `[tire-parse] Could not parse tire dimensions` fired for every `33x12.50R20LT`-style row. WheelPros writes flotation LT sizes with the service designation AFTER the rim diameter, and Pattern 2 ended at `(\d{2})\b` — with `LT` glued onto the digits there is no word boundary, so the row fell through all three patterns to an all-null result. Not cosmetic: a null `sizeToken` makes `tireSizeLabel()` fall back to the raw part number as the variant axis, and the tire indexes with no `canonical_size`, no rim/width/aspect facets and no `fit_specs` — i.e. unfindable by size. Two smaller variants of the same bug: load/speed written `123/Q` (slash) parsed as neither, and ag/implement sizes written `8.3X20 6PR` (X instead of dash, no R/B/D construction letter) matched no pattern at all.
- problem 2 — **~65 rows/run vanished from the staging summary.** `Staging complete:` accounted for staged + no-image + invalid-price + unreachable but NOT normalization failures, so a real run read `40,475 parsed / 31,432 staged / 6,705 / 0 / 2,273` and left 65 rows to be found by hand subtraction. Worse, each of those 65 logged `err.message` of a **ZodError**, which is its pretty-printed `issues` JSON — ~13 lines apiece, several hundred lines of noise per run burying the summary.
- fix: (1) Pattern 2 accepts an optional TRAILING `(LT|P|ST)`, captured into `tirePrefix` (so `classifyTireType` still reports light-truck) and excluded from `sizeToken`, while a LEADING prefix stays byte-identical so no already-indexed row changes; `parseLoadSpeedPly` allows an optional `/` before the speed letter; Pattern 3 captures its separator so `[-xX]` both work. All 11 distinct failing descriptions from the log now parse. (2) New pure `summarizeNormalizationError` collapses a ZodError to one line of `path: message` pairs (stable across rows, which is what makes aggregation possible); `stageFeed` caps per-row warnings at 5, then emits ONE aggregate ordered by frequency with a sample part number, and adds `skippedNormalizationCount` to both `StageResult` and the summary line so parsed now equals staged + every skip bucket.
- decision: the BoltPattern/Size rejections themselves were left INTACT — they are correct. Those rows are display units and accessories (`ASANTIDISPLAY`, `MO813MX20DISPLAY`, `28-46-081-113-3`) with a genuinely empty `BoltPattern`; a wheel with no bolt pattern cannot be fitment-matched and must not enter the catalog. The feed's deliberate "blank/undrilled" wheels use the literal token `BLANK`, which normalizes fine and is already suppressed at the facet layer by `isRealBoltPattern`. Only the logging changed.
- gate: backend jest `pnpm test:sync` **505 passing** (6 skipped; was 495 — +6 `normalization-error`, +4 `stage-feed` accounting/capping, +7 `tire-parse-helpers` incl. two explicit no-regression guards) + `tsc --noEmit` clean.
- **second pass (2026-07-27, after the storage fix let a real dry-run land):** the first pass only covered the formats in the user's pasted log. A live `vendor-sync:dry-run wheelpros-tires` against the real SFTP feed staged 2,616 rows and showed **215 (8.2%) still had a null `sizeToken`**, dominated by ONE family: inch flotation written with a dash instead of R (`33X12.50-22`, `LT35X12.50-20`) — 190 of the 215. Also added: slash-inch drag/crawler sizes (`29.0/10.5R18`, `17/49-20LT`, 21 rows) as a new Pattern 2b guarded so it can never steal a real metric size; the unicode multiplication sign (`37×12.50R18LT`) folded to X up front; `T` as a construction letter (`LT325/60T18`); `Z` on the dash-metric form (`265/35Z-22`); and trailing `XL`/`LT` plus a hyphen leading boundary on the metric pattern (`225/60R18XL`, `375/45R24LT`, `MOTIVO 365-245/45R20`). Measured against the real staged rows: **214 of 215 now parse (99.5%), 0 regressions** — the lone holdout is a golf-cart tire with a single-digit rim (`18X8.50-8`), left alone rather than weakening the 2-digit rim guard for one row.
- **⚠️ one-time canonicalization, must land in the SAME apply as WB-115:** the inch branch now REBUILDS `sizeToken` from its parts instead of echoing the match, so `LT37X12.50R18`, `37X12.50R18LT` and `LT37X12.50-18` all collapse to `37X12.50R18` (the designation moves to `tirePrefix`, which is what `classifyTireType` reads). That restates **179 of the 2,616** staged rows' tokens. Doing it in the same pass as the dead-image cleanup is deliberate: the Size option value IS the tire variant axis, so canonicalizing later would rewrite the axis on already-shipped variants — the axis-collision class this pipeline has been bitten by before. Metric keeps its leading prefix (`LT265/70R17`) unchanged; only the inch branch canonicalizes.
- gate (second pass): backend jest **518 passing** (was 505), `tsc --noEmit` clean, plus an out-of-band replay of the new parser over the 2,616 real staged rows (214/215 recovered, 0 regressions).
- not fixed here (needs the DB, which was storage-capped and in recovery all session): the 41 `no inventory_item_id` stock-apply warnings (0.15% of 27,940 parts) — pre-existing legacy rows with a documented one-shot remedy, `pnpm exec medusa exec ./src/scripts/vendor-sync-backfill-inventory.ts`.
- context: this session also traced a total storefront outage (every page a Server Components error) to the **Postgres storage limit**, not to any code change — backend `/health` 200 (no DB) while `/store/regions` and `/store/products` both hang, and a direct `psql` connect returns "the database system is in recovery mode". Resolves on storage upgrade; nothing to fix in the repo.

### WB-117 · Fitment UI: vehicle cache resurrects, no-fit vehicle still claims a fit, intermittent no-op submit   [DONE — code, UNVERIFIED IN BROWSER]
- status: done (code; merged to local main). Three user/tester-reported bugs, 2026-07-27. **Not reproduced in a live browser** — the storefront was down all session behind the Postgres storage limit (see WB-116), so these are code-level root causes with unit coverage, and each still wants a manual pass once the site is back.
- area: storefront/garage (`lib/garage/single-vehicle-garage.ts`) + storefront/search (`find-by-vehicle/ymm-pane.tsx`) + storefront/discovery + tire-discovery (`fitment-sync/strip-fit.ts` and both `fitment-sync/index.tsx`)
- **bug 1 — clearing a vehicle resurrects the previous one.** `LocalStorageGarage.remove()` ends with `writeActiveId(next[0]?.id ?? null)` — it promotes the first survivor, correct for the retired multi-vehicle garage, wrong under the WB-076 single-vehicle contract. Reachable whenever the stored list holds more than one row, i.e. a pre-WB-076 legacy cache that no `add()` has collapsed yet (`add()` is the ONLY thing enforcing the one-vehicle invariant, so a returning shopper who clears before ever picking a new car hits it). fix: `SingleVehicleGarage` now overrides `remove()` to be total — drop the row, drop any straggler, clear the active pointer. `add()` switched to `super.remove()` for its prune step, since `this.remove()` would now delete the vehicle it had just added (regression-tested).
- **bug 2 — a vehicle we have no data for was still saved and shown as active.** The `case "ok"` branch toasted "No fitment data for this vehicle yet" and then kept going: the vehicle stayed saved + active, the drawer closed, and it navigated to a destination with an EMPTY fit param — an unfiltered catalog. The discovery header is honest here (`isFitted` requires a real `fit` param), but the nav `GaragePill` lights up orange with "Vehicle · 2019 Ford Escape" purely on `active`, so the UI claimed a vehicle context the toast had just denied. fix: when the lookup succeeds but yields nothing usable for EITHER surface (no bolt patterns AND no OEM tire sizes), roll the vehicle back out of the cache, toast, and stay in the drawer without navigating. Deliberately distinct from the `unavailable`/`failed` branch, which still keeps the vehicle so "RE-CHECK FIT" can retry a transient outage — this is an authoritative "no data", where retrying returns the same nothing forever. A vehicle usable for only the OTHER surface is still kept and still navigates.
- **bug 3 — "Find My Fit" intermittently does nothing until a refresh.** ymm-pane deliberately skips `router.push` when already on the destination page and lets `FitmentSync` own the URL. But the YMM flow is add() → setActive() → *await lookup* → update(), and during that await the vehicle is active with no bolt patterns, so `FitmentSync` saw `desiredFit === null` and called `shouldStripFit({hasActive: false, …})` → **true** → `router.replace` stripping the very params the post-update pass re-adds a beat later. Two conflicting `router.replace` calls back to back — the same concurrent-navigation hazard ymm-pane already documents at its own `router.push` guard, where the progress bar starts and the URL never commits. fix: `shouldStripFit` gains an optional `fitmentPending`, set when an active vehicle's `canonicalBoltPatterns` (wheels) / `oemTires` (tires) is still `undefined` — never-written, as opposed to `[]` which means resolved-and-empty and IS a genuine orphan worth stripping. Applied to both the wheel and tire FitmentSync. Optional with a `false` default, so every other caller keeps its exact prior behavior.
- gate: storefront vitest **875 passing** / 121 files (+7: 3 `single-vehicle-garage` incl. a legacy-multi-row cache reproducing bug 1 and an `add()` regression guard, 4 `strip-fit` incl. an omitted-arg back-compat case) + `tsc --noEmit` at the documented **2-error baseline**.
- follow-ups: bug 3's fix removes the *known* double-navigation trigger, but "sometimes" was never reproduced live — if it recurs, instrument the two `router.replace` sites before assuming a new cause. `GaragePill` still shows an orange dot for any active vehicle regardless of whether a fit is applied; with bug 2 fixed the misleading combination is much rarer but the pill could still read "no fit data" explicitly.

## G13 · QA remediation (2026-07-28 external tester report)

> Findings + verification evidence: [plans/2026-07-29-qa-tester-report-triage.md](plans/2026-07-29-qa-tester-report-triage.md).
> Design: [specs/2026-07-29-qa-remediation-design.md](../in-progress/specs/2026-07-29-qa-remediation-design.md).
> `Q-NN` ids below refer to the register; each wave is one branch with its own review gate.

### WB-118 · Checkout & money integrity — $0 line total, summary math, shipping/tax config   [CRITICAL]
- status: in-progress (code DONE + gated; 2 tasks deferred — see `deferred:` below)
- area: storefront/cart + storefront/checkout + backend/scripts
- evidence: `storefront/src/lib/util/line-item-amounts.ts:29` — `item.unit_price ?? (total/quantity)`; `??` is nullish, so a `unit_price` of **0** never falls back and renders `$0.00` beside a correct line total (Q-01, the tester's "CART PRICE SHOWING 0 IN MAIN FIELD, SHOWING CORRECT PRICE BELOW THO"). Corroborated: checkout's own `LineItemRow` computes `perEa = total / quantity` and was NOT reported broken.
- problem: Q-01 $0 unit price · Q-02 checkout summary rows don't sum to the total (two different totals components — `cart-totals` on `/cart` vs `Totals` inside `checkout-summary` — five independent server fields, nothing asserting they reconcile) · Q-07 state field accepts free text ("Chicago" in the state box, no error) · Q-08 no postal code on the card form · Q-05 free-over-$199 never applied to prod · Q-06 every US state taxed identically.
- fix: **Task 1 captures a real cart/checkout payload as a fixture BEFORE any code changes** — Q-01/Q-02 are the only `UNVERIFIED` items and guessing at a money bug ships a second one. Then: derive the unit price when a contradictory zero meets a positive total; one shared pure `cartTotalRows(cart)` consumed by both surfaces; a validated US state select; Stripe postal-code collection. Ops half: run `update-shipping-prices.ts` on prod (Q-05 — client decision, Standard/Express stay same-priced for now, see WB-123), plus a NEW `--confirm-host`-guarded idempotent script creating province-level US tax regions from `state=rate` pairs (Q-06).
- verify: displayed rows sum to `cart.total` (invariant test over the Task-1 fixture + synthetic carts); two identical carts to Chicago IL vs Los Angeles CA return DIFFERENT tax; a basket either side of $199 flips shipping; postal code is collected. Live Stripe **test mode** only.
- blocked-on (does not block starting): the confirmed nexus state list + rates — an accounting determination, supplied to the script at run time.
- **done (code) 2026-07-29** on `feat/g13-qa-remediation`. Commits: capture `7afa364`, helper `13562c6`, /cart `9973127`, checkout+line totals `c3c1db2`, state field `b19b58d`, threshold+postal `e6de4f9`.
- **⚠️ Task 1 overturned the spec's Q-01 hypothesis.** The `$0.00` is the line **TOTAL**, not the unit price: the live Store API cart response carries **no `total` key on a line item at all** (key set is id/quantity/unit_price/tax_lines/adjustments/product/variant/…), so `item.total ?? 0` rendered $0.00 while `unit_price` (333) was correct. The tidy `??`-on-unit_price fix the spec proposed would have left the bug untouched. It was also present TWICE — `checkout-summary`'s own `LineItemRow` had the same line, which the plan had not anticipated. `lineItemAmounts` now derives each amount from the other, staying inside stored amounts.
- **Q-02 measured live before any fix:** displayed `343.00 + 11.00 + 34.30 = 388.30` against `cart.total 377.30` — overstated by exactly `shipping_total`, because `subtotal` already contains `shipping_subtotal` and `tax_total` already contains `shipping_tax_total` (`decorateCartTotals` lines 66/87/106/111-112). **The tester's "$11 ADDED ON EVERY SHIPPING" is this display bug, not a hidden fee.** Fixed by a single pure `cartTotalRows` consumed by both surfaces; `total` is always `cart.total` verbatim.
- **hardened beyond the plan:** `CartTotals` also renders the ORDER confirmation page, and `BaseOrder` declares neither `discount_subtotal` nor `credit_line_total` — a discounted order would have shown rows summing above its total. `cartTotalRows` falls back to `discount_total` and emits an explicit "Adjustments" row (+ dev-only `console.warn`) for any residual, so the arithmetic always closes.
- gate: storefront vitest **902 / 124 files** (was 875/121), tsc **2-error baseline** held, `next build` compiles; backend tsc 0.
- **deferred (NOT done):** Task 8 per-state tax script — deliberately held until the client answers, because [client-input-needed §1](../reference/client-input-needed.md) offers Stripe Tax as an alternative and a per-state script would be wasted work if they take it. Task 10 live verification — the fixes are local/undeployed, so the $388.30 → $377.30 correction has NOT been observed on a running page; it is proven only by the invariant test over the real captured cart.
- **⚠️ live findings needing the client, not code** ([client-input-needed.md](../reference/client-input-needed.md)): the production tax rule is literally named **"Defaul Tax rate For Testing"** at a flat **10%** on every US address (the mechanism behind Q-06); Standard and Express are **both $10.00**; and a live **$333** cart was still charged shipping despite the site advertising "Free shipping on orders $199+".
...
- fix: **persist-then-notify** — a `support-request` module mirroring `newsletter` (entity + migration + `POST /store/support-request`), because a submission that only sends an email is lost forever when email is down, and email is down right now. The storefront form writes the row first; notification is a best-effort side effect. Plus a static `mailto:`/`tel:` channel from `NEXT_PUBLIC_SUPPORT_EMAIL`/`NEXT_PUBLIC_SUPPORT_PHONE`, each rendering **only when set** (never a placeholder address). Q-20 reuses the same form with vehicle + product prefilled and `source: "fitment-check"`; the wheel PDP gains the equivalent CTA it currently lacks. Q-19 is proven end-to-end against a **stubbed transport** only — no live send.
...
- status: done (code) 2026-07-29 on `feat/g13-qa-remediation`
- area: storefront/discovery + storefront/tire-discovery + storefront/search + storefront/home
- evidence: `shop-by-style/style-map.ts:49` sums facet buckets (`values.reduce((sum, v) => sum + dist[v])`) over MULTI-VALUED `diameters`, so a wheel offered in 18" and 20" is counted twice. Measured live: Street 487+187+876 = **1550** claimed vs **1076** listed; Truck & Dually 733 vs 490; Drag 653 vs 593 — while Luxury / Off-road / UTV (the three SINGLE-value presets, where summing and distinct-counting coincide) match exactly. Separately, `discovery/.../filter-sections.tsx:279` `defaultValue={["brand","diameter","finish"]}` and `tire-discovery/.../filter-sections.tsx:279` `defaultValue={["brand","rim-diameter","tire-type"]}` leave exactly the sections the tester called "empty" collapsed — Radix unmounts collapsed content; the data is complete (46 bolt patterns, 500 tire sizes, 16 speed ratings, 80 load indexes all present in the page payload).
- problem: Q-12 + Q-03 style counts over-claim (homepage section AND `/styles`) · Q-10/Q-11 key filters read as empty · Q-03 search-drawer Trending tiles show out-of-stock products unbadged (`trending-data.ts` takes `newest.slice(0,3)` and DROPS `DiscoveryProduct.inStock`) · Q-13 brand count 25 vs 27 · Q-15 price inputs should be a range slider (already a `TODO(integration)` in the rail).
- **correction to the tester's report:** the `/store` and `/tires` "In stock only" toggles are NOT broken — measured 1447 → **1138** wheels (Meili ground truth 1137) and 611 → **399** tires, zero out-of-stock badges on either filtered page. Only the drawer lacks a gate.
- fix: a real distinct count (`estimatedTotalHits` of the same filter the listing applies) instead of a bucket sum; open Bolt pattern (`/store`) and Size (`/tires`) by default AND show the available-value count on every collapsed trigger, so a closed section can never again read as empty; prefer in-stock for the Trending tiles and carry `inStock` through so anything still shown is badged (deliberately NOT a hard filter — three badged tiles beat one tile); one identically-scoped brand-count helper; a price aggregation from Meilisearch, then the shadcn `Slider`.
- verify: every style tile count equals its listing count (a test pins a preset with overlapping values → distinct, not sum); a collapsed section reports its value count. Also confirm whether `tire_sizes` is truncated — it returns exactly **500** distinct values, which is the index's `maxValuesPerFacet: 500` ceiling.
- **done (code) 2026-07-29.** Commits: style counts `1b6447c`, filters + facet ceiling `896ee4d`, trending `038bab0`, price bounds `da86237`.
- **⚠️ NEW BUG FOUND while checking for truncation — the tyre Size filter was silently incomplete.** The facet returned exactly 500 values, suspiciously equal to `maxValuesPerFacet`. The live catalog carries **699 distinct `tire_sizes`**, so **199 sizes were absent from the filter entirely** and a shopper looking for one could never narrow to it. Ceiling raised 500 → **2000** in `medusa-config.js`. **This is an index SETTING: it needs a backend RESTART to apply — no reconcile, no re-import, the documents are unchanged.**
- **Q-13 NOT REPRODUCED, no code written.** Homepage, `/brands`, the `/store` rail and Meilisearch ground truth all read **25** for American Racing Forged; all three surfaces share one `facets.brands` source and cannot disagree. The catalog shifted between the tester's run and ours (1449 → 1447 results), which likely explains their 27.
- Q-12 root cause was arithmetic, and conclusively so: summing buckets of a MULTI-VALUED facet double-counts, and the only three styles that matched were exactly the three single-value presets where summing and distinct-counting coincide. `getStyleCounts` now issues one batched `multiSearch` reading exhaustive `totalHits` (`hitsPerPage`/`page`, not `limit`/`offset` — WB-088 D13), verified to return 1076 for STREET against the tile's claimed 1550. Summed counts survive only as an outage fallback so a failed query degrades to inaccurate tiles rather than a blank section.
- Q-10/Q-11 were never missing data — Radix unmounts collapsed accordion content and both rails omitted their primary dimension from `defaultValue`. Bolt pattern and Size now open by default, and EVERY trigger carries a value count so a collapsed section can never again read as empty.
- Q-15 shipped as **bounds only**: real `facetStats` min/max ($78–$2,450) now drive the price placeholders. The shadcn `Slider` is split out — it needs a new Radix dependency and pnpm is unreliable on PATH here. Bounds are the substance.
- gate: storefront vitest **941 / 128 files**, tsc **2-error baseline**, `next build` compiles; backend tsc 0.
- **deploy: backend RESTART required** (facet ceiling). No Meilisearch reconcile and no re-import — this wave changes how the index is queried, not what is in it.
- refs: [design Wave 3](../in-progress/specs/2026-07-29-qa-remediation-design.md)

### WB-121 · Catalog data honesty & routing — "Center bore 999 mm", uppercase paths 404   [MEDIUM]
- status: done (code) 2026-07-29 on `feat/g13-qa-remediation` (commit `202e8a4`)
- area: storefront/product-detail + storefront/middleware
- evidence: **45 wheel products** in the live index carry `center_bores: [999]` (Asanti Forged 832 / 862 / 868 among them) — a WheelPros sentinel for bore-to-order, rendered literally because `specs/spec-rows.ts:18` guards `centerBoreMm > 0`, which WB-056 added to hide 0/null and which 999 sails straight through. Separately: `/us/store/` → 308 → `/us/store` OK, but `/US/STORE` → 307 → `/us/STORE` → **404**, because `middleware.ts:108` lowercases only `pathname.split("/")[1]`.
- problem: Q-16 a plausible-looking fake spec (worse than a hidden row) · Q-17 any capitalised URL from an email or printed asset dead-ends.
- fix: **decided — storefront render layer, not ingest.** Normalising at ingest would need a full re-import + Meilisearch reconcile to correct 45 products; the sentinel surfaces in exactly two render sites (`spec-rows.ts`, `hero/index.tsx:286`), so one shared pure helper renders the honest "Custom / bore-to-order" there. Ingest normalisation is a follow-up folded into the next re-import. Middleware lowercases the whole pathname as a 308, composing with (not around) the WB-081 fail-open behaviour and the WB-096 X8 self-redirect guard.
- verify: a PDP for one of the 45 shows the honest row; **the fitment verdict for those 45 is UNCHANGED** — `fit-view.ts:51` `boreClears(999, hubBore)` currently clears every hub, which is arguably right for a bore-to-order wheel but is correct BY ACCIDENT and must not silently flip; `/US/STORE` resolves, `/us/store` does not redirect (no loop), query strings survive.
- **verification the plan demanded, satisfied by inspection:** `boreClears(bore, hub)` returns true for `bore == null` AND for `999 >= hub - tolerance`, so the fit verdict for those 45 products is provably identical whether the sentinel stays or goes. The fix is therefore DISPLAY ONLY and the data is untouched, which keeps that guarantee free. Do not "clean" 999 upstream without re-checking those verdicts.
- sentinel sweep (the tester asked, calling it "likely systemic"): **999 centre bore is the ONLY one.** `diameters` and `widths` are clean; `offsets` 0 (328 products) and -1 are legitimate wheel specs, not placeholders.
- rendered in BOTH surfaces, not just the spec table — some products carry a real bore AND the sentinel (Asanti Forged 862 is `[73.1, 999]`), so the hero's bore chips needed it too.
- **⚠️ Q-17 could not be fixed by lowercasing the pathname.** Deeper segments carry case-sensitive identifiers — above all the order ULID in `/order/confirmed/[id]`, which for a guest checkout is their ONLY route back to the order via the emailed link. Lowercasing it would turn a cosmetic 404 into a lost order. The fix is a bounded ALLOWLIST applied to the top-level route segment only; handles, slugs and ids keep their case, and an unknown segment is left alone so genuinely broken links still 404 rather than being silently rewritten. Pinned by a test asserting the ULID survives.
- gate: storefront vitest **954 / 130 files**, tsc **2-error baseline**, `next build` compiles.
- deploy: storefront rebuild only. No migration, no reconcile, no re-import.
- refs: [design Wave 4](../in-progress/specs/2026-07-29-qa-remediation-design.md)

### WB-122 · Search relevance   [DEFERRED — blocked on a repro]
- status: todo (blocked)
- area: storefront/search + backend/vendor-sync search
- evidence: the tester wrote only *"search bar needs to be improved"* — no failing query given.
- problem: not actionable as stated. WB-087 already added `search_text`, synonyms and model-name titles, so guessing at "better" risks regressing that work.
- fix: — (scope defined once a repro exists)
- verify: —
- unblocks-on: two or three concrete searches, each with the expected result and the actual result.
- refs: [triage Q-14](plans/2026-07-29-qa-tester-report-triage.md)

### WB-123 · Express shipping costs the same as Standard   [DEFERRED — blocked on carrier rates]
- status: todo (blocked)
- area: backend/scripts (shipping options)
- evidence: `update-shipping-prices.ts:65` treats `["Standard Shipping", "Express Shipping"]` identically; both are seeded flat.
- problem: a paid upgrade that costs the same as the free option. WB-118 applies the $199 free-shipping rule, so the ADVERTISED promise stops being false either way — but the method picker still offers a meaningless choice.
- fix: real per-method prices once rates exist.
