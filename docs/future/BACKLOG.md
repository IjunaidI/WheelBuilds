...
- **G6 · Catalog breadth & pricing** `[L–XL · WB-005 is a big spec alone]` — **IN PROGRESS (2026-07-02): WB-005 tires decomposed into 3 sub-projects; SP1 (backend grouping+indexing, merged) + SP2 (tire discovery `/tires`, on `feat/tire-discovery`) DONE** — tires group by Brand+model (size = variant axis), indexed as `product_type = "tire"`, and a parallel `tire-discovery` module renders a faceted `/tires` catalog (rim/size/type/speed/load). Remaining: SP3 (tire PDP) + prod cutover; then markup/MAP/margin pricing, de-hardcode bootstrap identity + vendor roster. → WB-005, WB-024, WB-025, WB-026
...
- fix: decomposed into 3 sub-projects (see spec). **SP1 (backend grouping + indexing) DONE + merged to `main`**: tires group by **Brand + extracted model** (per-SKU fallback), the canonical **size** is the variant axis, and a `buildSearchDocument` tire branch + Meili facets index them as `product_type = "tire"`. **SP2 (tire discovery) DONE** on branch `feat/tire-discovery`: a parallel `modules/tire-discovery/` module + `/tires` route with a faceted rail (rim/size/type/speed/load + brand/price), disjunctive-facet Meili data layer, tire product card, and Tires nav links — wheel discovery untouched, no fitment. SP3 (tire PDP) remains, plus the prod cutover (deploy → backend restart → tire feed apply). Tire fitment + MAP pricing explicitly out of scope.
- verify: after a tire feed apply, tires appear as grouped Medusa products with multiple size variants; Meilisearch contains tire documents with `product_type = "tire"` + tire facets; the `/tires` storefront route renders a faceted tire catalog. **SP1 unit gate: 288 backend tests. SP2 gate: storefront vitest 141 (24 files), tsc 0-new, `/tires` route compiles. Live dry-run/apply + a live `/tires` browse DEFERRED → pre-deploy (needs a real DB + tire feed apply).**
- refs: SP1 spec [tire-store design](../in-progress/specs/2026-07-02-tire-store-design.md) + plan [tire-store-backend](../in-progress/plans/2026-07-02-tire-store-backend.md) ; SP2 spec [tire-discovery design](../in-progress/specs/2026-07-02-tire-discovery-design.md) + plan [tire-discovery](../in-progress/plans/2026-07-02-tire-discovery.md)
