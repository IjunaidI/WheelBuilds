...
- **Brand-collection race — the one real hazard.** `brandCollectionCache` is a shared mutable `Map`; parallel groups for the same brand would double-create the collection. **Fix: serially pre-warm** all distinct brand collections (from `diff.newGroups` + `diff.changedGroups`) into the cache *before* the parallel phase, so tasks only read it. `ensureBrandCollection` stays as-is; a new `prewarmBrandCollections(ctx, diff)` populates the cache up front.
...
- Upload the archived CSV via the Medusa **File module** (`Modules.FILE`, already registered — MinIO when configured, local otherwise), storing the returned key in `vendor_feed_run.source_archive_key`.
