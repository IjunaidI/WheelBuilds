...
- **Brands/Style → `/store` interim repoint, labels kept.** `/collections` + `/categories` 404 today, so pointing at `/store` (+ style presets) strictly improves on dead links. Labels stay "Brands"/"Style"; this is an explicit **placeholder until WB-099** (dedicated brand/style landing pages) — flagged so it isn't mistaken for done.
