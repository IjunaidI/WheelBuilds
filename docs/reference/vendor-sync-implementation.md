...
**Off-request triggers.** The manual trigger (`POST /runs`) and approve/replay/replay-sku run their work off the HTTP request via the **event bus + a subscriber**, not in-request. The route validates + reserves the run (`startRun`), then **emits** a `vendor-sync.{execute,approve,replay,replay-sku}` event (resolving `Modules.EVENT_BUS` off `req.scope`) and returns immediately (201 for a new run, 202 for approve/replay/replay-sku). The subscriber [`src/subscribers/vendor-sync-run.ts`](../../backend/src/subscribers/vendor-sync-run.ts) runs the pipeline on **its own global container** — which, unlike the module-scoped constructor container (`this.container_`), can resolve the core region/product/inventory modules the apply workflows need (see [`resolve-apply-container.ts`](../../backend/src/modules/vendor-sync/pipeline/resolve-apply-container.ts)). It is deliberately event-driven rather than in-process `setImmediate`: the module cradle cannot resolve core modules, and a held `req.scope` would be disposed before a long apply finishes. The 12h and stock-tick crons still call the blocking service methods directly (they already hold the global container).
...
| [`service.ts`](../../backend/src/modules/vendor-sync/service.ts) | `VendorSyncService`: orchestrator + DB-backed cancel (`isCancelled`/`markCancelled`) + `startRun` (reserve) / `executeRun` (off-request pipeline body) / blocking `run` (cron) / `runStockOnly` / `approveAndApply` / `replayRun` / `replaySku` |
| [`../subscribers/vendor-sync-run.ts`](../../backend/src/subscribers/vendor-sync-run.ts) | Subscriber that runs `execute`/`approve`/`replay`/`replay-sku` off-request on its global container (WB-011/012/013) — the mechanism that makes the admin triggers async |
...
| POST | `/runs` | Trigger out-of-band run (body: `{ vendor_code, dry_run? }`) — **returns 201 immediately** (WB-011); emits a `vendor-sync.execute` event that the subscriber applies off-request on the global container |
