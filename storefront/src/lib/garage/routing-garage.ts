...
// RoutingGarage (this file) moved out of lib/garage/index.ts VERBATIM and out
// of the app import graph — only __tests__/routing-identity.test.ts imports it
// now, which keeps the whole local⇄remote sync machinery compiled and
// unit-tested while it's mothballed. To restore the garage, point the
// singleton in ./index.ts back at `new RoutingGarage()` and re-enable the
// other GARAGE-DISABLED seams (grep the marker).
