// storefront/src/modules/discovery/components/filter-rail/mobile-trigger-copy.test.ts
//
// WB-074 D2 review — MobileFilterTrigger renders on the same screen as
// DiscoveryHeader on viewports under `small`. Before this fix it always
// showed the raw `totalCount` ("200 results") even when fit mode's
// candidate cap made that count untrustworthy — directly contradicting the
// header's honest "TOP 200 CANDIDATES — REFINE TO NARROW" label shown at
// the same time. These pure label builders are what the component renders,
// extracted so the honest-label branch is unit-testable without a React
// render harness (no RTL/jsdom component-test infra in this repo — mirrors
// the get-products.cap.test.ts pure-function pattern).
import { describe, it, expect } from "vitest"
import { mobileTriggerLabel, mobileDrawerCta } from "./mobile-trigger-copy"
import { FIT_CANDIDATE_CAP } from "../../data/types"

describe("mobileTriggerLabel (WB-074 D2 review)", () => {
...
describe("mobileDrawerCta (WB-074 D2 review)", () => {
  it("shows the precise count when not capped (singular)", () => {
