...
describe("leadTimeLine", () => {
  it("special order overrides — the extended-lead-time warning, regardless of availability", () => {
    expect(
  ...
  it("out_of_stock but flagged SO -> still the SO warning (SO is a vendor-code fact, independent of on-hand qty)", () => {
    expect(
      leadTimeLine({ availability: "out_of_stock", isSpecialOrder: true })
    ).toBe(SPECIAL_ORDER_LEAD_TIME)
  })
})
