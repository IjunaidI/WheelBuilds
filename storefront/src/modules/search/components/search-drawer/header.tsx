...
const Header = ({ onClose }: HeaderProps) => {
  ...
  const submit = (e: FormEvent) => {
    ...
    // searching from /tires bounced to wheels, and a tyre-only brand
    // ("Falken": 65 tyres, 0 wheels) was unfindable.
    const extra: Record<string, string> = {}
    if (typeof window !== "undefined") {
      const currentFit = new URLSearchParams(window.location.search).get("fit")
      if (currentFit === "0") extra.fit = "0"
    }
    router.push(searchDestination(countryCode, pathname, trimmed, extra))
  }
