 ...
 * there are more than 7 pages. Copied verbatim from
 * modules/discovery/components/pagination — that component is route-agnostic
 * (it only calls the query hook's setPage, no hardcoded path), so the only
 * change here is pointing it at useTireQuery.
 */
const TirePagination = ({
  currentPage,
  totalPages,
}: TirePaginationProps) => {
  const { setPage } = useTireQuery()
